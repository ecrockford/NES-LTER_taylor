...
for count = 1:length(unq_combustrun)
    ...
    for count2 = 1:length(unqblank) %sometimes 2 diff blanks have the exact same combust and run dates, deal with this
        ...
        if CHNtable.event_number(ind2) == 'MVCO_461'
            CHNtable.blankfromblankfile(ind2) = {'none'};
            CHNtable.blankN(ind2) = 0;
            CHNtable.blankC(ind2) = 0;
            CHNtable.blank_estimated(ind2) = 1;
        else
